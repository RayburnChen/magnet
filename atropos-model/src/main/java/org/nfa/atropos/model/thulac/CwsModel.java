...
import static org.nfa.atropos.model.thulac.CwsModel.NGramFeature.BIGRAM_FEATURE_1;
import static org.nfa.atropos.model.thulac.CwsModel.NGramFeature.BIGRAM_FEATURE_2;
import static org.nfa.atropos.model.thulac.CwsModel.NGramFeature.BIGRAM_FEATURE_3;
import static org.nfa.atropos.model.thulac.CwsModel.NGramFeature.BIGRAM_FEATURE_4;
import static org.nfa.atropos.model.thulac.CwsModel.NGramFeature.SPACE;
import static org.nfa.atropos.model.thulac.CwsModel.NGramFeature.UNIGRAM_FEATURE_1;
import static org.nfa.atropos.model.thulac.CwsModel.NGramFeature.UNIGRAM_FEATURE_2;
import static org.nfa.atropos.model.thulac.CwsModel.NGramFeature.UNIGRAM_FEATURE_3;
import static org.nfa.atropos.model.thulac.Dat.MATCH_FAILURE_INDEX;
...
public final class CwsModel implements Serializable {
	...
	/**
	 * 特征对应某label的权重. Why use one-dimensional array but not two-dimensional array?
	 * Please Refer to <a>
	...
	/**
	 * 映射 enum POC 对应的所有label 索引值. 其中，行为POC的ordinal值，列为索引值
	 */
	 ...
	 *
	 * @param weightPath
	 *            label转移权重、特征label权重文件路径, cws_model.bin 或者model_c_model.bin
	 * @param featurePath
	 *            特征DAT文件路径, cws_dat.bin 或者 model_c_dat.bin
	 * @throws IOException
	...
	public CwsModel(String weightPath, String featurePath) throws IOException {
		...
		intBuffer = byteBuffer.order(ByteOrder.LITTLE_ENDIAN).asIntBuffer();
		...
		// Load label file
		List<String> labelList = IOUtils.readLines(IOUtils.getInputStream(ModelPaths.LABEL_PATH));
		...
		nsCementer = new DATCementer(IOUtils.getInputStream(ModelPaths.NS_BIN_PATH), "ns");
		idiomCementer = new DATCementer(IOUtils.getInputStream(ModelPaths.IDIOM_BIN_PATH), "i");
	}
	...
	/**
	 * N-gram 特征. THULAC采用的分词模型为结构化感知器（Structured Perceptron, SP），以最大熵准则
	 * 建模序列标注的得分函数.
	 */
	public static class NGramFeature {
		...
		/**
		 * Bigram 特征种类1，对应于特征 left + mid + SPACE + '1'， 即标注的前一字符加上当前字符
		 */
		public static final char BIGRAM_FEATURE_1 = '1';

		/**
		 * Bigram 特征种类2，对应于特征 mid + right + SPACE + '2'， 即标注对应的当前字符加上后一字符
		 */
		public static final char BIGRAM_FEATURE_2 = '2';

		/**
		 * Bigram 特征种类3，对应于特征 left2 + left1 + SPACE + '3'， 即标注的前二字符加上前一字符
		 */
		public static final char BIGRAM_FEATURE_3 = '3';

		/**
		 * Bigram 特征种类4，对应于特征 right + right2 + SPACE + '4'， 即标注的后一字符加上后二字符.
		 */
	 ...
	 */
	public int[] evaluateCharWeights(char left2, char left1, char mid, char right1, char right2, int[] labelIndices) {
