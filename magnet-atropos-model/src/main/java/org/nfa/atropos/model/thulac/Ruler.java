...
import static org.nfa.atropos.model.thulac.CharUtils.convertHalfWidth;
import static org.nfa.atropos.model.thulac.CharUtils.isDigit;
import static org.nfa.atropos.model.thulac.CharUtils.isLetter;
import static org.nfa.atropos.model.thulac.CharUtils.isRemainPunctuation;
import static org.nfa.atropos.model.thulac.CharUtils.isSinglePunctuation;
import static org.nfa.atropos.model.thulac.CharUtils.isSkipped;
import static org.nfa.atropos.model.thulac.CwsModel.NGramFeature.BOUNDARY;
...
public final class Ruler {
	...
	public static CleanedSentence ruleClean(String sentence, boolean isEnableTileWord) {
		...
		for (int i = 0; i < len;) {
				...
				if (isEnableTileWord) {
					...
					// 后书名号
						...
						if (isPossibleTitle(raw, titleBegin + 1, i - 1)) {
							setTitleWordPoc(cleanedSentence, titleBegin + 1, i - 1, cleanedSentence.length() - 2);
						}
						hasTitleBegin = false;
					}
				}
			...
			// 3. English or Latin words
				...
				i = processWord(raw, cleanedSentence, i, CharUtils::mayBeLetterWord, false);
			}

			// 4. Numbers
			else if (isDigit(ch)) {
				i = processWord(raw, cleanedSentence, i, CharUtils::mayBeNumeral, true);
			}
	 ...
	 *            词的结束位置（在句子中的index值）
	 * @param endIndex
	 *            wordEnd对应在result的index值
	 */
	private static void setTitleWordPoc(CleanedSentence result, int wordStart, int wordEnd, int endIndex) {
		// 单独字符成词
	 ...
	 */
	private static int processWord(char[] sentence, CleanedSentence result, int wordStart, Predicate<Character> condition, boolean isNumeral) {
		POC b, m, e, s;
		if (isNumeral) {
			b = POC.BEGIN_M_POC;
			m = POC.MIDDLE_M_POC;
		...
		i++;
		// 单独成词
		if (i == len || (i < len && !condition.test(sentence[i]))) {
